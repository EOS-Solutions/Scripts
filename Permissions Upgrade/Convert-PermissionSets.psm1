...
function DownloadSymbolsFromDatabase
{
    ...
    $SymbolsBytes = (Invoke-Sqlcmd -ServerInstance $DatabaseServer -Database $DatabaseName -Username $DatabaseUsername -Password $DataBasePassword -Query "Select [Symbols] from [Published Application] where ID = '$PackageId'" -MaxBinaryLength $PackageSize).Symbols
...
function Convert-PermissionSets
{
    ...
    $Apps = Invoke-Sqlcmd -ServerInstance $DatabaseServer -Database $DatabaseName -Username $DatabaseUsername -Password $DataBasePassword -Query 'Select ID, Name, Datalength(Symbols) as Size from [Published Application]'
    ...
    foreach($App in $Apps)
    {
        DownloadSymbolsFromDatabase -DatabaseServer $DatabaseServer -DatabaseName $DatabaseName -DatabaseUsername $DatabaseUsername -DataBasePassword $DataBasePassword -PackageId $App.ID -PackageSize $App.Size -Path (Join-Path $SymbolsFolder ($App.Name))
    }
    
    $Symbols = ProccessSymbols $SymbolsFolder

    Write-Host "Quering permissions from $DatabaseServer $DatabaseName"
    $Permissions = Invoke-Sqlcmd -ServerInstance $DatabaseServer -Database $DatabaseName -Username $DatabaseUsername -Password $DataBasePassword -Query 'Select [Role ID],[Object Type],[Object ID],[Read Permission],[Insert Permission],[Modify Permission],[Delete Permission],[Execute Permission],[Security Filter] from Permission UNION Select [Role ID],[Object Type],[Object ID],[Read Permission],[Insert Permission],[Modify Permission],[Delete Permission],[Execute Permission],[Security Filter] from [Tenant Permission] order by [Role ID]'
    $PermissionSets = @{}
    ...
    $PermissionSetTableContent = Invoke-Sqlcmd -ServerInstance $DatabaseServer -Database $DatabaseName -Username $DatabaseUsername -Password $DataBasePassword -Query 'Select [Role ID], Name from [Permission Set] UNION Select [Role ID], Name from [Tenant Permission Set]'
